...
import EventReminderModal from '../sidebarMenuPages/workspace/event/_components/EventReminderModal';
import EditEventModal from '../sidebarMenuPages/workspace/event/_components/EditEventModal';
...
const Events = () => {
  ...
  useEffect(() => {
    const fetchEventCount = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/event/events/total`, {
          method: 'GET',
          headers: {
        ...
        setEventCount(data.total_events); // Assuming `data` is a number for task count
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAllEvents = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/event/user/events`, {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        ...
        <div className="flex">
          <div className="flex flex-col lg:gap-2">
            ...
            <div className="flex justify-start items-center space-x-2 mb-4">
              ...
              <p className="text-slate-600">{eventcount} tasks</p>
            </div>
          </div>
        </div>
