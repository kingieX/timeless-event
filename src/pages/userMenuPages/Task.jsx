...
import EditTaskModal from '../sidebarMenuPages/workspace/task/_components/EditTaskModal';
import TaskReminderModal from '../sidebarMenuPages/workspace/task/_components/TaskReminderModal';
import UpdateAccessModal from '../sidebarMenuPages/workspace/task/_components/UpdateAccessModal';
import UpdateStatusModal from '../sidebarMenuPages/workspace/task/_components/UpdateStatusModal';
import UpdatePriorityModal from '../sidebarMenuPages/workspace/task/_components/UpdatePriorityModal';
...
const Task = () => {
    ...
    const fetchTaskCount = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/task/task/get-total-count-of-task-by-user`,
          {
            method: 'GET',
            headers: {
        ...
        setTaskCount(data); // Assuming `data` is a number for task count
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAllTasks = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/task/task/all-created-by-user`,
          {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <>
      <div className="w-full lg:p-10 py-1 lg:px-8 px-4">
        {/* Header Section */}
        <div className="flex">
          <div className="flex flex-col lg:gap-2">
            ...
            <div className="flex justify-start items-center space-x-2 mb-4">
              ...
              <p className="text-slate-600">{taskcount} tasks</p>
            </div>
          </div>
        </div>
